...
Die vorstehende Tabelle fasst noch einmal alle zuvor aufgeführten Virtualisierungsansätze zusammen. Deren unterschiedliche Vor- und Nachteile sollen nun noch einmal vergleichend diskutiert werden.

Bei der Installation der verschiedenen Ansätze kann man grundsätzlich zwischen denen unterscheiden, die sich als natives Betriebssystem auf nakter Hardware installieren lassen und denen, die man als zusätzliches Anwendungsprogramm installiert. Grundsätzlich ist die Installation als Anwendungsprogramm einfacher und erfordert zum Beispiel weniger Zugriffsrechte. Außer bei der Virtualisierung mittels OS-Container ist diese Installationsvariante aber auch tendenziell weniger effizient.

Bis auf das Produkt VMWare vSphere handelt es sich bei allen Ansätzen um kostenlose Open-Source-Produkte. Dafür bietet WMWare vSphere mit der Live-Migration von laufenden virtuellen Maschinen zwischen verschiedenen Hosts aber auch eine sehr mächtige Funktion, die sich gerade für große und flexible Virtualisierungsszenarien anbietet.

Grundsätzlich lassen sich mit den meisten Ansätzen verschiedene Betriebssysteme (zum Beispiel Windows und Linux) virtualisieren. Ausnahmen stellen hier lediglich MirageOS und Docker dar, da MirageOS-Unikernels kein Betriebssystem beinhalten und Docker als OS-Container-Lösung lediglich Anwendungen virtualisiert und kein Betriebssystem.

Das Ausführen eines PHP-Interpreters innerhalb einer virtuellen Maschine ist ohne weiteres bei allen Ansätzen außer MirageOS möglich.

Docker bietet mit seinem Union-Filesystem im Gegensatz zu den anderen Ansätzen eine Möglichkeit, die Nutzung des Dateisystems zu optimieren.

In Hinblick auf die Problemstellung lässt sich zunächst nur MirageOS als mögliche Virtualisierungslösung ausschließen, da die Ausführbarkeit eines PHP-Interpreters zumindest für die Ausgangssituation bei der Pixelhouse GmbH zwingend notwendig ist.
