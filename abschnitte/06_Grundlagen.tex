...
"`Unit-Tests verify the behaviour of small elements in a software system, which are most often a single class."' \citep[S.][S. 132]{DuvMatAnd07} Unit-Tests konzentrieren sich darauf, dass Verhalten einer konkreten Klasse sowohl in Bezug auf ihren internen Zustand als auch in Bezug auf ihr Zusammenspiel mit anderen Klassen zu testen \citep[Vgl.][S. 7]{microservicetesting}. Eine wichtige Eigenschaft von Unit-Tests ist dabei, dass sie keine externen Abhängigkeiten wie zum Beispiel Datenbank oder Dateisystem besitzen. Tests können damit sehr früh und schnell geschrieben werden und benötigen nur wenig Zeit zur Ausführung \citep[Vgl.][S. 133]{DuvMatAnd07}. Sie benötigen deshalb aber auch keine anspruchsvolle Testumgebung oder eine Optimierung der Ausführungszeit. Für die Problemstellung dieser Arbeit sind sie also weniger interessant.
...
Akzeptanz- bzw. Funktionale Tests unterscheiden sich von System-Tests insofern, als das bei ihnen die Betonung darauf liegt, das System aus Sicht des Benutzers zu testen \citep[Vgl.][S. 136]{DuvMatAnd07}. Ein Beispiel für so einen Test ist das manuelle Testen der Anwendung durch einen Projekt- oder Produktmanager. Es ist außerdem möglich, automatische Tests zu schreiben, die die Anwendung über einen programmatisch gesteuerten Webbrowser überprüfen. Solche Tests werden gerne mit Werkzeugen geschrieben, die es ermöglichen, die einzelnen Schritte des Tests in natürlichsprachlicher Syntax zu schreiben. Dadurch soll es zum Beispiel einem Produktmanager oder sogar Kunden möglich sein, die Tests zu verstehen und zu validieren. Die Abbildung \ref{behat} zeigt einen solchen Test, den die Pixelhouse GmbH im Rahmen ihrer Qualitätssicherung mit dem Werkzeug Behat \citep[][]{Behat15} benutzt, um das Einloggen auf der Website Chefkoch.de zu testen.

\begin{figure}[!ht]
  \begin{center}
    \begin{gherkincode}
Feature: Login
    In order to use personalized features
    As a website user
    ...
    Scenario: Login succesfully
        ...
        When I follow "Login"
        And fill in "tester" for "Benutzername / E-Mail"
        And fill in "tester" for "Passwort"
        And press "Einloggen"
        Then I should see "tester"
        And should see "Logout"
    \end{gherkincode}
    \caption{Testscenario für das Einloggen auf Chefkoch.de}
    \label{behat}
  \end{center}
\end{figure}

Genau wie der System-Tests benötigt der Aktzeptanz-Test auf jeden Fall das gesamte System und somit auch eine vollständige Testumgebung. Auch die Dauer der Testausführung ist wie bei Systemtests höher als die von Unit- und Integrations-Tests.
...
\begin{figure}[!ht]
  \begin{center}
    \includegraphics[width=14cm]{bilder/Einordnung_Virtualisierungstechnologien_für_virtuelle_Betriebsumgebungen.png}
...
Ein Bare Metal Hypervisor ist ein \ac{VMM}, der als eigenständiges Programm direkt auf echter physischer Hardware läuft und somit kein Betriebssystem auf dem Hostsystem benötigt. In der Abbildung \ref{bare} ist zudem zu sehen, dass auf einem entsprechenden System kein gewöhnliches Betriebssystem oder normale Anwendungsprogramme existieren.
...
Bei VMWare vSphere handelt es sich um ein kommerzielles Produkt der Firma VMWare, dem Marktführer im Bereich der Virtualisierungslösungen mit einem Marktanteil von ungefähr 50\% \citep[Vgl.][]{vmware:001}.

vSphere basiert auf einem von VMWare eigens entwickelten Betriebssystem, dem sogenannten VMkernel \citep[Vgl.][S. 3]{vmware:002}. Wie in Abbildung \ref{vmware} zu sehen, laufen auf diesem Betriebssystem das Programm \ac{DCUI}, eine Konsole zur direkten Konfiguration des Servers, und diverse weitere Schnittstellen, mit denen man die Verwaltung des Servers und der auf ihm laufenden virtuellen Maschinen auch über das Netzwerk erledigen kann. Für jede virtuelle Maschine wird ein eigener Hypervisor- und ein weiterer Hilfsprozess gestartet (VMM und VMX).

vSphere bietet die Möglichkeit, virtuelle Maschinen im laufenden Betrieb von einem vSphere Server auf einen anderen zu übertragen. Diese Funktion, vMotion genannt, erleichtert das Resourcenmanagement enorm, da sich eine laufende Anwendung auf andere Hardware übertragen lässt, um die vorherige Hardware zum Beispiel zu reparieren oder auch einfach nur zur Einsparung auszuschalten \citep[Vgl.][S. 4]{wmware:003}.
...
Auch bei XenServer von der Firma Citrix handelt es sich um einen Bare Metal Hypervisor, der direkt auf der eigentlichen physischen Hardware aufsetzt \citep[Vgl.][]{xen:001}. XenServer basiert ebenfalls auf einem Linux-Kernel. Es handelt sich aber nicht lediglich um Kernel-Module, die sich in eine beliebige Linux-Distribution laden lassen, sondern um einen modifizerten Linux-Kernel. XenServer lässt sich also nur auf komplett leere Hardware installieren und bietet auch nicht die vollständigen Funktionalitäten einer normalen Linux-Distribution. Wie in der Abbildung \ref{xen} zu sehen, ist das Besondere an XenServer, dass die Prozesse zur Steuerung der virtuellen Maschinen selbst in einer virtuellen Maschine laufen. Diese auch "`Privileged Guest"' genannte Maschine muss laufen, bevor weitere Gastsysteme geladen werden können. Der "`Privileged Guest"' oder auch DOM0 genannte Gast hat dabei (wie der Name sagt) besondere Rechte, die ihm die Steuerung der darunterliegenden Virtualisierungsschicht im Hypervisor ermöglichen \citep[Vgl.][S. 2]{Schee14}.
...
Als Antwort entstehen deshalb in jüngster Vergangenheit neue Lösungen, bei denen man seinen eigentlichen Programmcode mit Hilfe einer Library entwickelt, die einfache Schnittstellen zur Verfügung stellt, um zum Beispiel Netzwerkkommunikation oder ähnliches zu betreiben. Der Programmcode wird dann mit Hilfe dieser Library in eine sehr viel kleinere \ac{VM} kompiliert, die wieder mit Hilfe eines Hypervisors ausgeführt werden kann.

Ein Beispiel für eine solche Library ist MirageOS \citep[Vgl.][]{MadMorAnd13}. Der Ansatz dieser Library wird in Abbildung \ref{unikernel} gezeigt.
...
Damit die einzelnen Container dennoch isoliert laufen und weder das Hostsystem noch andere Container manipulieren können, muss der Kernel spezielle Schnittstellen anbieten. Entsprechende Technologien sind im Laufe der Zeit bei einer Reihe von Betriebssystemen entwickelt worden, zum Beispiel auch für Linux und Windows.
